...
public interface RestauranteRepository extends JpaRepository<Restaurante, Integer> {
    ...
    @Query(value ="select r.idrestaurante, r.nombre, r.foto, r.fotocontenttype, r.fotonombre \n" +
            ...
            "where r.nombre like %?1% and r.estado = 1 \n" +
            "and (t2.`prom_prec` >=?2 and t2.`prom_prec` < ?3) \n" +
            "and (t.`prom_val` > ?4 and t.`prom_val` <=?5) \n" +
            "having  t3.`idcategorias` like %?6% \n" +
            "or t3.`idcategorias` like %?7 \n" +
            "or t3.`idcategorias` like ?8% order by r.iddistrito = ?9 DESC ", nativeQuery = true)
    Page<RestauranteDTO> listaRestaurante2(String texto, Integer limitInfP, Integer limitSupP,Integer limitInfVal, Integer limitSupVal, String id1,String id2, String id3,Integer iddistrito, Pageable pageable);
